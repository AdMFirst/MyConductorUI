...
function buildTask(node: any, index: number): ConductorTask | null {
  ...
  if (node.type === 'http') {
    ...
    return {
      ...
      description: "Generated by MyConductorUI",
      retryCount: 0
    }
  }
  ...
  if (node.type === 'simple') {
    // implementing simple register is out of scope so i just put it like this
    // afaik simple need to be registered to conductor separatly
    return {
      name,
      taskReferenceName: reference,
      type: 'SIMPLE',
